...
- 客服为自建在线客服系统，入口不放在 Tab 上，降低审核风险。
- 登录按需触发，不开屏强制登录。
...
- `pages/index/index`：首页，服务说明、搜索入口、客服入口、商品服务入口、热门商品。
...
- `pages/profile/index/index`：我的，登录、预约、商品服务、客服、隐私入口。
...
- 客服入口来源支持 `home / goods_list / product_detail / order_detail / profile / tab`。
...
- 隐私说明页只做说明，不再额外放“同意隐私”按钮，避免和抖音原生隐私流程冲突。
...
- 价格数字统一使用金色 `#C8964E`。
- 首页、商品列表、详情、预约、我的、隐私、客服页全部统一视觉层级。
- 底部 Tab 选中态同步为新主色。
...
注意：当前机器没有 `git` / `gh`，GitHub 连接器不支持直接按本地路径上传二进制文件，因此本协作仓库当前同步的是进度、交接清单、校验值和接口边界。后续建议把 `D:\dyxcx\自提商城` 初始化为独立前端代码仓库。
...
- 客服接口需要后端确认最终消息模型、上传返回字段、离线同步行为和坐席身份展示规则。
